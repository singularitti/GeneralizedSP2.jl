...
!!! info
    The style guide is always a work in progress, and not all GeneralizedSP2 code
    follows the rules. When modifying GeneralizedSP2, please fix the style violations
...
GeneralizedSP2 uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/GeneralizedSP2.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the GeneralizedSP2 directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to GeneralizedSP2 have
