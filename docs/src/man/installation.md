...
[GeneralizedSP2](https://github.com/singularitti/GeneralizedSP2.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("GeneralizedSP2")
   ```

3. Run

   ```julia-repl
   julia> using GeneralizedSP2
   ```
...
version of GeneralizedSP2, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/GeneralizedSP2.jl"
```
...
our [GitHub repository](https://github.com/singularitti/GeneralizedSP2.jl)
for new releases.
Once we release a new version, you can update GeneralizedSP2 by typing

```@repl
using Pkg
Pkg.update("GeneralizedSP2")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("GeneralizedSP2")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall GeneralizedSP2.
