...
@testset "Test computing the model's gradients" begin
    for backend in
        (AutoEnzyme(; mode=Reverse), AutoEnzyme(; mode=Reverse, function_annotation=Const))
        ...
        @testset "Custom example" begin
            ...
            x = 0.35
            derivatives = Array{Float64}(undef, size(model))
            @test manualdiff_model!(identity, derivatives, model, x) ≈ [
            ...
            @test autodiff_model(identity, model, x, backend) ≈
                manualdiff_model(one, model, x)
            @test autodiff_model(x -> x^2 / 2, model, x, backend) ≈
                manualdiff_model(identity, model, x)
        end
    end
end
